                ...
                with tracer.start_as_current_span("agent.remediation") as fix_span:
                    ...
                    try:
                        remediation_prompt = (
                            ...
                            f"Generate fixes for ALL of the following prioritized security findings.\n"
                            f"For EACH finding you MUST:\n"
                            f"1. Call `generate_fix` to get fix guidance\n"
                            f"2. Call `validate_fix` to validate your fix\n"
                            f"3. Call `assess_confidence` to get the confidence score\n"
                            f"4. MANDATORY: Call `create_review_with_suggestions` with:\n"
                            f"   - repo=\"{repo}\"\n"
                            f"   - pr_number={pr_number}\n"
                            f"   - commit_sha=\"{head_sha}\"\n"
                            f"   - suggestions_json=<your JSON array of suggestions>\n\n"
                            f"DO NOT skip calling `create_review_with_suggestions`.\n"
                            f"DO NOT just describe the fix in text.\n\n"
